...
def generate2optNeighbours(tour):
    ...
    for i in range(1, cities):
        for j in range(i+1, cities):
            if ((i == 1) and (j == cities-1)): continue
            # print(i,j,tour[i+1:j+1])
            neighbour = tour[0:i] + tour[i:j+1][::-1] + tour[j+1:cities]
...
def generate3optNeighbours(tour):
    ...
    for i in range(0,cities):
        for j in range(i+2,cities):
            for k in range(j+2,cities-1+(i>0)):
                t1 = tour[0:i+1] + tour[i+1:j+1][::-1] + tour[j+1:k+1][::-1] + tour[k+1:cities]
                t2 = tour[0:i+1] + tour[j+1:k+1][::-1] + tour[i+1:j+1] + tour[k+1:cities] 
                t3 = tour[0:i+1] + tour[j+1:k+1] + tour[i+1:j+1] + tour[k+1:cities]
                t4 = tour[0:i+1] + tour[j+1:k+1] + tour[i+1:j+1][::-1] + tour[k+1:cities]
                all_possible_neighbours.append(t1)
                all_possible_neighbours.append(t2)
                all_possible_neighbours.append(t3)
                all_possible_neighbours.append(t4)
...
def hillClimbUtil(tour, getNeighbours):
    ...
    all_possible_neighbours = getNeighbours(tour)

    for neighbour in all_possible_neighbours:
        cost = getTourLength(neighbour)
        if cost < minTourCost:
            minTourCost = cost
            minTour = neighbour
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    cost = getTourLength(initial_tour)    
    minTour, minTourCost = hillClimbUtil(initial_tour, getNeighbours)

    while minTourCost < cost:
        cost = minTourCost
        tourLengthList.append(cost)
        minTour, minTourCost = hillClimbUtil(minTour, getNeighbours)

    tourLengthList.append(cost)  # comment this line for task 4   
...
def nearestNeighbourTour(initial_city):
    ...
    currCity = initial_city
    tour.append(currCity)
    remaningCities = list(range(1,cities+1))
    remaningCities.remove(currCity) 

    while (len(remaningCities) != 0):
        distances = [getDistance(nodeDict[currCity],nodeDict[nextCity]) for nextCity in remaningCities]
        nearestCityIndex = distances.index(min(distances))
        nearestCity = remaningCities[nearestCityIndex]
        tour.append(nearestCity)
        currCity = nearestCity
        remaningCities.remove(nearestCity)
...
def finalOrder(mst, initial_city):
    ...
    while (not stack.isEmpty()):
        top = stack.pop()
        fin_order.append(top)
        for edge in mst[::-1]:
            if ((edge[0] == top) and (edge[1] not in fin_order)): stack.push(edge[1])
            elif ((edge[1] == top) and (edge[0] not in fin_order)): stack.push(edge[0])
