			...
			for i in range(len(trainingData)):
				...
				if (guess != trainingLabels[i]):
					for l in self.legalLabels:
						if (trainingLabels[i] < l and self.weights[frozenset([trainingLabels[i], l])] * trainingData[i] > 0):
							self.weights[frozenset([trainingLabels[i], l])] = self.weights[frozenset([trainingLabels[i], l])] - trainingData[i]
						if (trainingLabels[i] > l and self.weights[frozenset([l, trainingLabels[i]])] * trainingData[i] <= 0):
							self.weights[frozenset([l, trainingLabels[i]])] = self.weights[frozenset([l, trainingLabels[i]])] + trainingData[i]
